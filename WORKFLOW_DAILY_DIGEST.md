...
1. `drafts/YYYY-MM-DD/` 전체를 git add, commit(예: `Draft: YYYY-MM-DD 다이제스트 N건`), push한다.
2. PushNotification으로 짧게 알린다. 예: "오늘자 다이제스트 초안 3건 준비됨 (drafts/YYYY-MM-DD) — 검토 후 발행 요청해주세요"
3. 여기서 끝. 발행(사이트에 실제로 반영하는 것)은 보스가 로컬에서 승인한 뒤 별도 도구로 처리되니, 이 루틴이 직접 발행하지 않는다.

## 예외 처리

- 오늘 의미 있는 이슈가 3건 미만이면 억지로 채우지 말고 있는 만큼만 만들고, 알림에 사유를 남긴다.
- 특정 소스에 접속이 안 되면 다른 소스로 대체하고 어떤 소스가 실패했는지 알림에 남긴다.
- git push가 실패하면(충돌 등) 강제 push하지 말고 실패 사실을 알림에 남긴다.
